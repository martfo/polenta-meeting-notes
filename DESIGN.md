...
An existing recording can also be imported from Settings or by dropping it on the window: any
audio the Mac reads (mp3, m4a, a WAV at another rate) is converted to the vault's 16 kHz mono
PCM on the way in (pipeline.audio_io.ensure_16k_mono_wav, afconvert) and then runs the
